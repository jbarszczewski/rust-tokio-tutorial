...
```
Please notice that we are acquiring lock on our balance but we don't need to unlock it manually. Once the `MutexGuard` returned by `lock()` goes out of scope, the lock is automatically removed. In our situation it is straight after we're done with preparing response contents. 

Now we are ready to run our app and test if we are getting proper balance response. Example cURL command:
```curl
